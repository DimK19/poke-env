...
import random
import time
from poke_env.player.tools import _get_damage_calc, _get_max_damage_move

URL_DAMAGE_CALC = "http://localhost:8060/calculate"

class NDGreedyPlayer2(Player):
    """
    ...
    async def play_out_1v1(self, me: Pokemon, opp: Pokemon, my_hp_override = None):
        ...
        mybm, my_dmg = await _get_max_damage_move(
            myobj,
            oppobj,
            self._foo[mekey]['moves']
        )

        oppbm, opp_dmg = await _get_max_damage_move(
            oppobj,
            myobj,
    ...
    async def choose_move(self, battle):
        ...
        if(not battle.available_moves or not battle.active_pokemon):
            ## Choose switch
            ## bring in a mon that can kill opp
            ## if none make random move
            eligible = []
            #print(f'NDG makes switch because there is no mon in')
            #print('=' * 50)
            for cand in battle.available_switches:
                iwin, _ = await self.play_out_1v1(cand, battle.opponent_active_pokemon)
                if(iwin):
            ...
            if(eligible):
                return self.create_order(random.choice(eligible))
            else:
                return self.choose_random_move(battle)
        ...
        ###print(f'BOOSTS = {active_mon._boosts}')
        # --- Step 0: Play out current 1v1 ---
        iwin, turns = await self.play_out_1v1(active_mon, opp_active)
        #print(f'NDG: {active_mon._species} VS {opp_active._species}', end = ' ')
        ...
        if battle.available_switches:
            if iwin:
                ...
                # Probability formula: max{60, 100 - 10 * turns to lose}
                if random.uniform(0, 100) < max(60, 100 - (10 * turns)):
                    should_switch = True
                else:
                    #print('Roll decided no switch')
                    pass
        ...
        if should_switch:
            ...
            oppobj = {
                ...
                'ability': self._foo[oppkey]['ability'],
                'evs': self._foo[oppkey]['evs'],
                'boosts': battle.opponent_active_pokemon._boosts
            }
            ...
            myobj = {
                ...
                'boosts': battle.active_pokemon._boosts
            }

            oppmove, oppdmg = await _get_max_damage_move(oppobj,
                                                        myobj,
                                                        self._foo[oppkey]['moves']
                                                        )
            ...
            for candidate in battle.available_switches:
                ...
                switch_in_dmg = _get_damage_calc(
                    spec = oppkey,
                    item = self._foo[oppkey]['item'],
                    ...
                    opp_boosts = battle.opponent_active_pokemon._boosts
                )
            ...
            if candidates:
                ...
                #print(f'!!!!!! NDG decides to switch to {best_candidate}')
        ...
        if random.random() < p_opp_stay:
            ...
            best_move, _ = await _get_max_damage_move(myobj, oppobj, list(map(lambda x: x._id, battle.available_moves)))
